...
def draw_rgb_3dboxes(reader, rgb_out=None):
    ...
    for data in reader:
        ...
        print(idx)
        img = draw_rgb_3dbox(rgb, cal, res, (0,0,255))
        img = draw_rgb_3dbox(img, cal, objs, (0,255,0))
        #save
        if rgb_out:
...
def draw_rgb_2dboxes(reader, rgb_out=None):
    ...
    for data in reader:
        ...
        img = draw_rgb_2dbox(img, cal, objs, (0,255,0))
        #save
        if rgb_out:
            #cv2.imwrite( os.path.join(rgb_out, idx+'.png'), img)
            image = Image.fromarray(cv2.cvtColor(img,cv2.COLOR_BGR2RGB))
...
def draw_lidar_3dboxes(reader):
    ...
    for data in reader:
        ...
        fig = draw_lidar_fig(fig, velo, res, cal,(1,0,0), drawlidar=False,GT=True)
        mlab.view(azimuth=135, elevation=70, focalpoint=[ 22.0909996 , -1.04700089, -2.03249991], distance=62.0, figure=fig)
        #mlab.show()
        mlab.savefig( filename='1116/%06d.pdf'%int(idx))
        mlab.close()
    input()
...
def draw_lidar_fig(fig, velo, objs, calib, c=(1,1,1), drawlidar=True,lidar_color=None, GT=False):
    ...
    for obj in objs:
        ...
        #print(box_pc.shape)
        if GT:
            draw_lidar_pc(box_pc, (1., 0.7,0), fig, pts_mode='sphere')
